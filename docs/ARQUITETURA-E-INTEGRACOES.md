...
**Etiqueta e filtro por formato:** cada post/vídeo no painel ganhou uma etiqueta visível ("Reels"/"Post" no Instagram, "Short"/"Vídeo" no YouTube) e um filtro "Todos / Reels/Shorts / Posts/Vídeos" acima da lista, que funciona junto com o seletor de rede no topo da página (ex.: YouTube + "Reels/Shorts" mostra só os Shorts do canal). O painel "Resumo de publicações" também passou a indicar a rede atual no canto (`postsStatsNetworkPill`) em vez do rótulo fixo "Instagram".

`followers-dashboard.js` passou a ler os dois documentos e juntar as listas (cada item marcado com `network: 'Instagram'`/`'YouTube'`), estendendo o painel "Melhores posts" — antes exclusivo do Instagram — para aceitar também o YouTube (`POSTS_NETWORKS`). O painel de insights de follows/unfollows/alcance continua exclusivo do Instagram (`isInstagramOrAllSelected`), pois esse dado não existe para as demais redes. Métricas que o YouTube não tem (interações totais, salvamentos, Reels/estático) aparecem como "—", mesmo tratamento já usado para posts do Instagram sem `/insights` disponível. Facebook e TikTok ainda não têm coleta de conteúdo.
