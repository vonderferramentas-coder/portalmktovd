...
Eles usam o segredo `META_PAGE_ACCESS_TOKEN` nos **GitHub Actions Secrets** e publicam dados agregados/públicos do Instagram e do Facebook da própria marca (nunca dados pessoais de terceiros) em `data/social-followers-live.json`, `data/social-followers.json`, `data/social-posts.json` e `data/social-facebook-posts.json`. Atualmente, apenas a marca padrão VONDER tem coleta automática (Instagram, Facebook e YouTube); demais marcas e redes são manuais no painel.
