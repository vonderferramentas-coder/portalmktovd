...
- `diagnostico-meta-posts.yml`: verifica alcance/permissões do token para dados de posts (mídia e insights por post), sem alterar arquivos — foi o que confirmou que o token atual já alcança `like_count`/`comments_count` na listagem de mídia e `total_interactions`/`views`/`saved` em `/insights` (o nome legado `engagement` foi descontinuado pela Meta). Também conta, paginando `/media` inteiro, quantos posts a Graph API enxerga no total, para comparação manual com a contagem de publicações do app do Instagram — sonda a suspeita (ainda não confirmada, 10/09/2026) de que posts de Collab recebidos de outra conta (a VONDER convidada, não a iniciadora) não aparecem em `/media`, já que o dono do objeto de mídia na Graph API é sempre quem inicia o Collab;
