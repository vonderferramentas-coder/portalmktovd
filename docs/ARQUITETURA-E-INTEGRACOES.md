...
Seguindo o mesmo padrão de `sync-meta-posts.yml` (Instagram), `sync-youtube-videos.yml` (a cada 6h) coleta os ~30 vídeos mais recentes do canal Vonder com `YOUTUBE_API_KEY` (dados públicos, sem OAuth): acha a playlist de uploads do canal (`channels.list`), lista os vídeos dela (`playlistItems.list`) e pede curtidas/comentários/visualizações em lote (`videos.list`). Grava em `data/social-youtube-videos.json` e publica em `portalStore/youtube-videos-vonder-v1` — um documento Firestore próprio, separado de `posts-vonder-v1` (Instagram), para uma coleta nunca apagar o snapshot da outra.
