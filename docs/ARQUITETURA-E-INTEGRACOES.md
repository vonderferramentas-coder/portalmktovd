...
**Shorts vs vídeos normais:** o canal publica Shorts quase todo dia — os primeiros 30 uploads mais recentes ficavam só de Shorts, sem espaço pros vídeos "normais" (mais espaçados) aparecerem. A paginação até 150 itens resolve isso. Para distinguir os dois tipos, já que a API do YouTube não expõe um campo "isto é um Short", usa a duração como heurística: vídeos com até 3 minutos (mesmo teto que o próprio YouTube usa para Shorts desde 2024) viram `mediaProductType: 'REELS'`, os demais `'VIDEO'` — reaproveitando exatamente o campo que `sync-meta-posts.yml` já usa para Reels do Instagram.
