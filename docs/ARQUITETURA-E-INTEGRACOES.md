...
- `diagnostico-meta-facebook-historico.yml`: sonda quais métricas de Insights de fãs/seguidores da Página ainda respondem (`page_fans`, `page_follows`, `page_daily_follows_unique` etc.), sem alterar arquivos — necessário porque a Meta descontinuou várias métricas de fãs ao longo dos anos e a métrica usada para reconstruir o histórico do Instagram (`follows_and_unfollows`) não existe para Páginas;
