...
- `diagnostico-meta-facebook-historico.yml`: sonda quais métricas de Insights de fãs/seguidores da Página ainda respondem (`page_fans`, `page_follows`, `page_daily_follows_unique` etc.) e quais escopos (`scopes`) o token atual tem via `debug_token`, sem alterar arquivos — necessário porque a Meta descontinuou várias métricas de fãs ao longo dos anos, a métrica usada para reconstruir o histórico do Instagram (`follows_and_unfollows`) não existe para Páginas, e a interface do Meta Business Suite (aba "Tendências" de "Público") mostra um gráfico diário de seguidores que a API nega — sinal de possível permissão (`read_insights`) faltando no token, a checar antes de descartar a reconstrução;
